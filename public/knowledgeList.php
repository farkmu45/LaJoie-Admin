...
$knowledgeList = Knowledge::getDetail($categoryId);

if ($_GET['deleteId']) {
  if (Knowledge::deleteDetail($_GET['deleteId'])) {
...
<head>
  ...
  <title>Manajemen Knowledge &rsaquo; <?= $categoryName ?> &mdash; LaJoie</title>
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1><?= StringUtil::decodeUrl($categoryName) ?></h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active">
                <a href="knowledge.php">Manajemen Knowledge</a>
              </div>
              <div class="breadcrumb-item"><?= StringUtil::decodeUrl($categoryName) ?></div>
            </div>
          </div>
                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table class="table table-striped" id="table-1">
                        ...
                        <tbody>
                          <!-- Knowledge List -->
                          <?php foreach ($knowledgeList as $key => $knowledge) { ?>
                            <tr>
                              <td class="text-center"><?= $key + 1 ?></td>
                              <td><?= $knowledge['name'] ?></td>
                              <td>
                                <?= StringUtil::limit($knowledge['text']) ?>
                                <div class="table-links">
                                  <a href="knowledgeDetail.php?id=<?= $knowledge['id'] ?>">View</a>
                                </div>
                              </td>
                              <td>
                                <a href="knowledgeEdit.php?id=<?= $knowledge['id'] ?>" class="btn btn-icon icon-left btn-primary"><i class="fas fa-edit"></i>Edit</a>
                                <a href="knowledgeList.php?category_id=<?= $categoryId ?>&name=<?= $categoryName ?>&deleteId=<?= $knowledge['id'] ?>" class="btn btn-icon icon-left btn-primary"><i class="fas fa-trash"></i>Delete</a>
                              </td>
                            </tr>
                          <?php } ?>
                          <!--  -->
                    ...
                    <div class="card-footer text-right">
                      <div class="buttons">
                        <a href="addKnowledge.php?category_id=<?= $categoryId ?>&name=<?= StringUtil::encodeUrl($categoryName) ?>" class="btn btn-icon icon-left btn-primary"><i class="fas fa-plus"></i>Add</a>
                      </div>
                    </div>
                  </div>
                </div>
