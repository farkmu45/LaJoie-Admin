...
if ($_POST) {
  ...
  $name = $_POST['name'];
  $password = empty($_POST['password']) ? $admin['password'] : password_hash($_POST['password'], PASSWORD_BCRYPT);

  if (Admin::update($admin['id'], $name, $username, $password)) {
    header('Location: userEdit.php?id=' . $admin['id']);
  }
}
...
<head>
  ...
  <title>Manajemen User &rsaquo; <?= $admin['username'] ?> &mdash; LaJoie</title>
